...
protected:
  ...
  /// vector's storage.
    ...
    // Use std::less to avoid UB.
		std::less<> LessThan;
		return !LessThan(First, this->begin())
		       && !LessThan(Last, First)
  ...
  /// Check whether any part of the range will be invalidated by clearing.
	...
	void assertSafeToReferenceAfterClear(ItTy From, ItTy To)
	{
		if constexpr (
		    ...
		    && std::
		        is_same_v<std::remove_const_t<std::remove_pointer_t<ItTy>>, std::remove_const_t<T>>
		) {
			if (From == To)
				return;
...
protected:
  ...
  /// starting with "Dest", constructing elements into it as needed.
	...
	{
		if constexpr (
		    std::is_pointer_v<It1>
		    && std::is_pointer_v<It2>
		    && std::is_same_v<
...
struct CalculateSmallVectorDefaultInlinedElements {
  ...
  // bytes.
	static constexpr size_t PreferredInlineBytes = kPreferredSmallVectorSizeof
	                                               - sizeof(SmallVector<T, 0>);
