...
struct DenseMapInfo<T *> {
  ...
  //               "Log2MaxAlign bits of alignment");
	...
	{ return (unsigned((uintptr_t) PtrVal) >> 4) ^ (unsigned((uintptr_t) PtrVal) >> 9); }
...
struct DenseMapInfo<std::pair<T, U>> {
	...
	static unsigned getHashValue(const Pair &PairVal)
	{
		return detail::combineHashValue(
		    FirstInfo::getHashValue(PairVal.first), SecondInfo::getHashValue(PairVal.second)
		);
	}
  ...
  // to combine hash values manually
	...
	{
		return detail::combineHashValue(
		    FirstInfo::getHashValue(First), SecondInfo::getHashValue(Second)
		);
	}
...
struct DenseMapInfo<std::tuple<Ts...>> {
	...
	static constexpr Tuple getEmptyKey() { return Tuple(DenseMapInfo<Ts>::getEmptyKey()...); }
	...
	{
		return (
		    DenseMapInfo<std::tuple_element_t<Is, Tuple>>::isEqual(
...
struct DenseMapInfo<std::optional<T>> {
	...
	{
		return detail::combineHashValue(
		    OptionalVal.has_value(), Info::getHashValue(OptionalVal.value_or(Info::getEmptyKey()))
		);
	}
