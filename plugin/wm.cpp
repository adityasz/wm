...
using Config::Lua::Bindings::Internal::checkResult;
using Config::Lua::Bindings::Internal::configError;
...
{
	if (!lua_istable(L, 1))
		return configError(L, "{}: expected a table {{class=, cmd=}}", name);
...
extern "C" __attribute__((visibility("default"))) std::tuple<std::string, std::string, std::string, std::string> PLUGIN_INIT(void * handle)
{
	auto die = [&handle](const auto &msg) {
	...
	if (std::strcmp(compositor_hash, client_hash)) {
		die(std::format(
		    "Version mismatch (headers ver = {} != {} = running ver)", compositor_hash, client_hash
		));
	}

	if (Config::mgr()->type() != Config::CONFIG_LUA)
		die("legacy config is not supported");
	...
	if (!register_functions(handle))
		die("failed to register functions");
...
{ g_pHyprRenderer->m_renderPass.removeAllOfType("AppSwitcherPassElement"); }

extern "C" __attribute__((visibility("default"))) std::string PLUGIN_API_VERSION() { return "0.1"; }
